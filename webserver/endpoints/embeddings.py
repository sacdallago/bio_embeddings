...
from webserver.tasks.embeddings import get_embedding
...
class Embeddings(Resource):
    ...
    def post(self):
        ...
        job = get_embedding.apply_async(args=[sequence], time_limit=60*5, soft_time_limit=60*5, expires=60*60)
        embedding = job.get()

        buffer = io.BytesIO()

        np.savez_compressed(buffer, embedding)
