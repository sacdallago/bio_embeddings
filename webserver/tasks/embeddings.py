...
def load_models():
    ...
    models['seqvec'] = SeqVecEmbedder(weights_file=_weight_file,
...
def get_embedding(sequence, embedder='seqvec'):
    try:
...
@task_keeper.task(time_limit=60*5, soft_time_limit=60*5, expires=60*60)
def get_features(sequence, embedder='seqvec'):
    try:
        model = models.get(embedder, models['seqvec'])
